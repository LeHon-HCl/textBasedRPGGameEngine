...
- [ ] 回滚按钮（rollback + session 重建）+ 历史回看（react-window）
- [ ] 快捷键映射（1-9 选项/S/L 快存读/H 历史）+ 设置页说明
- [ ] 成就图鉴面板（隐藏占位/进度条/收集率）
- [ ] 调试面板（变量监视/跳转/时间快进/表达式控制台/评估日志，manifest 开关）
- [ ] 判定呈现（check_result 动画，减弱动画降级）+ 战斗面板（OQ-04：嵌入叙事区形态）
