...
- **遗留登记**：E2E 战斗流用例依赖宿主战斗 UI（25B 战斗面板），登记为 25 号
  B 组验收面——引擎级全链已由 battle-flow.test.ts 覆盖。宿主消费 battle jump
  的通路已就绪（battle_start 事件 + createBattleController + jumps 回流，
  #33），UI 侧接面板即可。
...
- **设计偏差裁定（2026-09-19 人类审查通过，约束 2 闭环）**：
  ...
  ③ AI `when` 表达式的战斗内作用域桥接：**已完成**（#39，2026-09-19）——
  battle.* v1 清单获人类确认并落地（域定义见下），接线层 evalCondition 实装
  （编译缓存 + 行动者上下文），条件型 AI 可用；demo 数据按 W6 片约定暂未写
  条件策略（后续可 additive 补）。
