...
> 分工与认领：见 `docs/plans/M2-stage2-battle-split.md`（双人协作，先认领后动工）。
...
> 实施计划（并行工作包拆解 / 接口草案）：`docs/plans/M2-stage4-achievements-loop-plan.md`（2026-09-21 人类裁定：全单线 + 内部并行）。
...
> 实施计划（Q0 前置核实结论 / 并行工作包 / 两处待决策口径）：`docs/plans/M2-stage5-qol-plan.md`（2026-09-23 人类裁定：全单线 + 内部并行；先五后六）。
